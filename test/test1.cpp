...
mystl::string txFormatT(double t) { //天象时间格式化输出
...
void tianXiang(int xm, int xm2, Date dat, int n = 10) {
    mystl::static_array<mystl::string, 9> xxName = {"地球", "水星", "金星", "火星", "木星", "土星", "天王星", "海王星",
                                                    "冥王星"};
...
void pCalc(int xt, Date dat, JINGWEI jw, int n = 10, int dt = 1, bool Cd_ut = true) { //行星星历计算
...
void suoCalc(int y, int n = 24, int jiao = 0) {//定朔测试函数
    y -= 2000;
    if (jiao == -1) {
...
void qiCalc(int y, int n = 24) {//定气测试函数
    y -= 2000;
    int i;
    double T;
    mystl::string s = "", s2 = "";
...
void houCalc(int y, int n = 24) {//定候测试函数
    y -= 2000;
    int i;
    double T;
...
void dingQi_cmp(int y = 2000, int N = 10) {//定气误差测试
    y -= 2000;
    int i;
    double W, T, maxT = 0;
    for (i = 0; i < N; i++) {
    ...
    std::cout << "\033[31;1m" << to_str(2000 + y) << "年之后" << to_str(N) << "个朔日粗算与精算的最大差异:"
              << to_str(maxT) << "秒。\033[0m" << std::endl;
}

void dingSuo_cmp(int y = 2000, int N = 10) { //定朔测试函数
    y -= 2000;
    int i;
    double T, maxT = 0, W;
...
void ML_calc(Date dat, JINGWEI jw) {
    ...
    std::cout <<
              "\033[31;1m[日标]：\033[0m" << "公历 " << dat.Y << "-" << dat.M << "-" << dat.D << " 儒略日数 "
...
void initmap(OB_LUN lun) {
    mystl::map<mystl::string, mystl::string> str_yx = {{"望",   "\033[33m●"},
                                                       {"上弦", "\033[33m∪"},
                                                       {"朔",   "\033[38;5;245m●"},
                                                       {"下弦", "\033[33m∩"}};
    for (int i = 0; i < 18; i++) {
