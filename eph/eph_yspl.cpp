...
double YS_PL::lineT(RE0 G, double v, double u, double r, bool n) {//已知t1时刻星体位置、速度，求x*x+y*y=r*r时,t的值
...
void YS_PL::lecXY(double jd, RE0 &re) {//日月黄经纬差转为日面中心直角坐标(用于月食)
    double T = jd / 36525;
    mystl::array3 zm = {}, zs = {};

    //=======太阳月亮黄道坐标========
    zs = e_coord(T, -1, -1, -1);   //地球坐标
    zs[0] = rad2mrad(zs[0] + _pi + gxc_sunLon(T));
...
void YS_PL::lecMax(double jd) { //月食的食甚计算(jd为近朔的力学时,误差几天不要紧)
    //YS_PL::lT={};
    ...
    YS_PL::lecXY(jd, G);
    YS_PL::lecXY(jd + dt, g); //精密差分得速度,再求食甚
    u = (g.y - G.y) / dt;
    v = (g.x - G.x) / dt;
