...
void MSC::calc(double T, double L, double fa,
               double high) {                                //sun_moon类的成员函数。参数：T是力学时,站点经纬L,fa,海拔high(千米)
    //基本参数计算
    ...
    if (fabs(rad2rrad(MSC::mCJ - MSC::sCJ)) < 50.0 / 180.0 * _pi) {
        COORDP pp = lineEar({MSC::mCJ, MSC::mCW, MSC::mR}, {MSC::sCJ, MSC::sCW, MSC::sR * cs_AU}, MSC::gst);
        MSC::zx_J = pp.J;
