...
void RS_GS::init(double jd, int n) { //创建插值表(根数表)
...
mystl::array3 RS_GS::chazhi(double jd, int xt) {//日月坐标快速计算(贝赛尔插值法),计算第p个根数开始的m个根数
...
mystl::array3 RS_GS::cd2bse(mystl::array3 z, mystl::array3 I) { //赤道转贝塞尔坐标
    mystl::array3 r = {z[0] - I[0], z[1], z[2]};
    r = llrConv(r, -I[1]);
    return llr2xyz(r);
}

mystl::array3 RS_GS::bse2cd(mystl::array3 z, mystl::array3 I) { //贝塞尔转赤道坐标
...
mystl::array3 RS_GS::bse2db(mystl::array3 z, mystl::array3 I, bool f) { //贝赛尔转地标(p点到原点连线与地球的交点,z为p点直角坐标),f=1时把地球看成椭球
    mystl::array3 r = xyz2llr(z);
    r = llrConv(r, I[1]);
...
mystl::array3
RS_GS::bseXY2db(double x, double y, mystl::array3 I, bool f) { //贝赛尔转地标(过p点垂直于基面的线与地球的交点,p坐标为(x,y,任意z)),f=1时把地球看成椭球
    double b = f ? cs_ba : 1;
    COORDP F = lineEar2(x, y, 2, x, y, 0, b, 1, I);//求中心对应的地标
    return {F.J, F.W};
}

mystl::array3 RS_GS::bseM(double jd) {  //月亮的贝塞尔坐标
...
_VXY RS_GS::Vxy(double x, double y, double s, double vx, double vy) { //地球上一点的速度，用贝塞尔坐标表达，s为贝赤交角
    _VXY r = {};
    double h = 1 - x * x - y * y;
...
_RSM RS_GS::rSM(double mR) { //rm,rs单位千米
    _RSM re = {};
...
mystl::array3 RS_GS::qrd(double jd, double dx, double dy, bool fs) { //求切入点
...
_FEATURE RS_GS::feature(double jd) {//日食的基本特征
...
mystl::array4 RS_GS::nanbei(mystl::array3 M, double vx0, double vy0, double h, double r,
                            mystl::array3 I) { //vx0,vy0为影足速度(也是整个影子速度),h=1计算北界,h=-1计算南界
...
bool RS_GS::mDian(mystl::array3 M, double vx0, double vy0, bool AB, double r, mystl::array3 I,
                  mystl::vector<double> &A) { //日出日没食甚
    double R;
    NODE p;
    mystl::array3 a = M;
    _VXY c = {};
    for (int i = 0; i < 2; i++) { //迭代求交点
...
mystl::string RS_GS::jieX3(double jd) { //界线表
    double k, ls;
    mystl::array4 p;
    int i;
