...
//大距计算
double xingJJ(int xt, double t, int jing) {                                //行星的距角,jing为精度控
    mystl::array3 a, z;
...
mystl::array2 daJu(int xt, double t, bool dx) {                                //大距计算超底速算法, dx=1东大距,t儒略世纪TD
    double a = 0, b, c[5] = {};
    if (xt == 1) {
...
mystl::array3 xingLiu0(int xt, double t, int n, double gxs) {                                //行星的视坐标
    mystl::array3 a, z;
    mystl::array2 zd;
    double E = hcjj(t);
...
double xingLiu(int xt, double t, bool sn) {                                //留,sn=1顺留
...
//合月计算
mystl::array4 xingMP(int xt, double t, int n, double E, mystl::array4 g) {                                //月亮行星视赤经差
    mystl::array3 a, p, m;
    ...
    m = llrConv(m, E + g[3]);
    p = llrConv(p, E + g[3]);
    mystl::array4 re = {rad2rrad(m[0] - p[0]), m[1] - p[1], m[2] / cs_GS / 86400 / 36525.0,
                        p[2] / cs_GS / 86400 / 36525.0 * cs_AU};    //赤经差及光行时
    return re;
}

mystl::array4 xingHY(int xt, double t) {                                //行星合月(视赤经),t儒略世纪TD
    double i, v, E;
...
//合冲日计算(视黄经合冲)
mystl::array4
xingSP(int xt, double t, int n, double w0, double ts, double tp) {                                //行星太阳视黄经差与w0的差
    mystl::array3 a, p, s;
...
mystl::array2 xingHR(int xt, double t, bool f) {                                //xt星体号,t儒略世纪TD,f=1求冲(或下合)否则求合(或下合)
    mystl::array4 a, b;
    double i, v, dt = 2e-5;
...
mystl::string xingX(int xt, double jd, double L, double fa) { //行星计算,jd力学时
    //基本参数计算
...
//========日月食计算使用的一些函数=============
COORDP lineEll(double x1, double y1, double z1, double x2, double y2, double z2, double e,
               double r) { //求空间两点连线与地球的交点(靠近点x1的交点)
...
COORDP lineEar2(double x1, double y1, double z1, double x2, double y2, double z2, double e, double r,
                mystl::array3 I) { //I是贝塞尔坐标参数
...
COORDP lineEar(mystl::array3 P, mystl::array3 Q, double gst) { //在分点坐标中求空间两点连线与地球的交点(靠近点P的交点),返回地标
...
NODE cirOvl(double R, double ba, double R2, double x0, double y0) { //椭圆与圆的交点,R椭圆长半径,R2圆半径,x0,y0圆的圆心
    NODE re = {};
...
_ECFAST ecFast(double jd) { //快速日食搜索,jd为朔时间(J2000起算的儒略日数,不必很精确)
    _ECFAST re = {};
    double t, t2, t3, t4;
    ...
    ls1 = Agm - (b + r1);
    if (fabs(ls1) < 0.016) re.ac = 0; //无食分界
    ls2 = Agm - (b + Ar2);
    if (fabs(ls2) < 0.016) re.ac = 0; //偏食分界
    ls3 = Agm - (b);
    if (fabs(ls3) < 0.016) re.ac = 0; //无中心食分界
    ls4 = Agm - (b - Ar2);
    if (fabs(ls4) < 0.016) re.ac = 0; //有中心食分界(但本影未全部进入)
