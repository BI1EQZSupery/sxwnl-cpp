...
mystl::string rysCalc(Date d, bool is_utc, bool nasa_r, JINGWEI jw) {
    ...
    if (msHJ > 170 / radd && msHJ < 190 / radd) {// 月食图表放大计算
        ...
        s = "本影半径 " + m2fm(er, 2, 0) + " 半影半径 " + m2fm(Er, 2, 0) +
            " 月亮地心视半径 " + m2fm(mr, 2, 0) + "\n" + "影月中心距 " + m2fm(d1, 2, 0) +
            " 影月半径和 " + m2fm(d0, 2, 0) + " \n距相切 \033[31m" + m2fm(d1 - d0, 2, 0) + "\033[0m 距第二相切 " +
            m2fm(d1 - d2, 2, 0);
...
// if(fs==2) ; //保持时间不变
    ...
    mystl::map<mystl::string, mystl::string> lxb = {{"T",  "全食"},
                                                    {"A",  "环食"},
                                                    {"P",  "偏食"},
                                                    {"T0", "无中心全食"},
                                                    {"T1", "部分本影有中心全食"},
    ...
    if (fs == 1 || fs == 2 || fs == 3 || fs == 4) {
        ...
        if (r.lx == "N") { s = "无日食"; }
            ...
            s = s + "\n"
                ...
                + "食分=" + to_str(r.sf, 4) + ", 食延=" + m2fm(r.tt * 86400, 0, 2) + ", 食带=" + to_str(r.dw, 0) +
                "km\n"
                + "\n";
        }
        // std::cout<<s<<std::endl;
...
mystl::string shengjiang3(int y) {    //年度时差
    Date dt = {y, 1, 1, 12};
    double jd = toJD(dt);
    int i;
    double t, D;
