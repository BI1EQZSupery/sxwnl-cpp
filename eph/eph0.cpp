...
double rad2rrad(double v) {//对超过-PI到PI的角度转为-PI到PI
    v = fmod(v, (2 * _pi));
    if (v <= -_pi)
        return v + 2 * _pi;
    if (v > _pi)
...
mystl::array3 llr2xyz(mystl::array3 JW) {//球面转直角坐标
    mystl::array3 r = {};
...
mystl::array3 xyz2llr(mystl::array3 xyz) {//直角坐标转球
    mystl::array3 r = {};
...
mystl::array3 llrConv(mystl::array3 JW, double E) {//球面坐标旋转
    //黄道赤道坐标变换
    mystl::array3 r = {};
...
mystl::array3 CD2DP(mystl::array3 z, double L, double fa, double gst) {//赤道坐标转为地平坐标
...
double j1_j2(double J1, double W1, double J2, double W2) {//求角度差
...
mystl::array3 h2g(mystl::array3 z, mystl::array3 a) {//日心球面转地心球面,Z星体球面坐标,A地球球面坐标
...
double shiChaJ(double gst, double L, double fa, double J, double W) { //视差角(不是视差)
...
double hcjj(double t) {//返回P03黄赤交角,t是世纪数
...
double dt_calc(double y) {//计算世界时与原子时之差,传入年
    double dt_at[] = {
...
double dt_T(double t) {//传入儒略日(J2000起算),计算TD-UT(单位:日)
    return dt_calc(t / 365.2425 + 2000) / 86400.0;
}

double prece(double t, const char *ss, const char *mx) {//t是儒略世纪数,s是岁差量名称,mx是模型
    char s[4] = {};
    strcat(s, ss);
    strcat(s, " ");
    int n = 0;
...
//==========================岁差旋转==========================
mystl::array3 CDllr_J2D(double t, mystl::array3 llr, const char *mx) {//J2000赤道转Date赤道
...
mystl::array3 CDllr_D2J(double t, mystl::array3 llr, const char *mx) {//Date赤道转J2000赤道
    double Z = -prece(t, "z", mx) + llr[0];
    double z = -prece(t, "Z", mx);
    double th = -prece(t, "th", mx);
    double cosW = cos(llr[1]), cosH = cos(th);
...
mystl::array3 HDllr_J2D(double t, mystl::array3 llr, const char *mx) {//黄道球面坐标_J2000转Date分点,t为儒略世纪数
...
mystl::array3 HDllr_D2J(double t, mystl::array3 llr, const char *mx) {//黄道球面坐标_Date分点转J2000,t为儒略世纪数
    mystl::array3 r = {llr[0], llr[1], llr[2]};
    r = llrConv(r, prece(t, "E", mx));
    r[0] += prece(t, "x", mx);
    r = llrConv(r, -prece(t, "w", mx));
...
double MQC(double h) {//大气折射,h是真高度
...
mystl::array3 parallax(mystl::array3 z, double H, double fa, double high) {//视差修正
...
mystl::array2 nutation2(double t) {//中精度章动计算,t是世纪数
...
/*高精度算章动*/
mystl::array2 nutation(double t, int zq) {//章动计算,t是J2000.0起算的儒略世纪数,zq表示只计算周期天于zq(天)的项
...
double nutationLon2(double t) {//只计算黄经章动
...
/*******************计算行星位置********************/
double XL0_calc(int xt, int zn, double t, int n) {//xt星体,zn坐标号,t儒略世纪数,n计算项数
    t /= 10;//转为儒略千年数
    int i, j;
    double v = 0, tn = 1, c;
    double *F = XL0[xt];
...
mystl::array3 pluto_coord(double t) {// 返回冥王星J2000直角坐标
...
mystl::array3 p_coord(int xt, double t, int n1, int n2, int n3) {//xt星体,T儒略世纪数,TD
    mystl::array3 z = {};
    if (xt < 8) {
...
mystl::array3 e_coord(double t, int n1, int n2, int n3) {//返回地球坐标,t为世纪数
    mystl::array3 re = {};
...
double XL1_calc(int zn, double t, int n) {//计算月亮
...
mystl::array3 m_coord(double t, int n1, int n2, int n3) {//返回月球坐标,t为世纪数
    mystl::array3 re = {};
...
double gxc_sunLon(double t) {//太阳光行差,t是世纪数
    double v = -0.043126 + 628.301955 * t - 0.000002732 * t * t;    //平近点角
    double e = 0.016708634 - 0.000042037 * t - 0.0000001267 * t * t;
    return (-20.49552 * (1 + e * cos(v))) / rad;    //黄经光行差
}

double gxc_sunLat(double t) {//黄纬光行差
    return 0;
}

double gxc_moonLon(double t) {//月球经度光行差,误差0.07"
    return -3.4E-6;
}

double gxc_moonLat(double t) {//月球纬度光行差,误差0.006"
    return 0.063 * sin(0.057 + 8433.4662 * t + 0.000064 * t * t) / rad;
}

double E_Lon(double t, int n) {//地球经度计算,返回Date分点黄经,传入世纪数、取项数
    return XL0_calc(0, 0, t, n);
}

double M_Lon(double t, int n) {//月球经度计算,返回Date分点黄经,传入世纪数,n是项数比例
    return XL1_calc(0, t, n);
}

double pGST(double T, double dt) {//传入T是2000年首起算的日数(UT),dt是deltatT(日),精度要求不高时dt可取值为0
...
double pGST2(double jd) {//传入力学时J2000起算日数，返回平恒星时
    double dt = dt_T(jd);
    return pGST(jd - dt, dt);
}

double E_v(double t) {//地球速度,t是世纪数,误差小于万分3
    double f = 628.307585 * t;
    return 628.332 + 21 * sin(1.527 + f) + 0.44 * sin(1.48 + f * 2) + 0.129 * sin(5.82 + f) * t +
           0.00055 * sin(4.21 + f) * t * t;
}

double M_v(double t) {//月球速度计算,传入世经数
...
double S_aLon_t(double W) {//已知太阳视黄经反求时间
...
double MS_aLon_t2(double W) {//已知月日视黄经差求时间,高速低精度,误差不超过600秒(只验算了几千年)
    double t, v = 7771.37714500204;
    t = (W + 1.08472) / v;
    double L, t2 = t * t;
    t -= (-0.00003309 * t2 + 0.10976 * cos(0.784758 + 8328.6914246 * t + 0.000152292 * t2) +
          0.02224 * cos(0.18740 + 7214.0628654 * t - 0.00021848 * t2) - 0.03342 * cos(4.669257 +
                                                                                      628.307585 * t)) / v;
    L = M_Lon(t,
              20) - (4.8950632 + 628.3319653318 * t + 0.000005297 * t * t + 0.0334166 * cos(4.669257 + 628.307585 * t) +
                     0.0002061 * cos(2.67823 + 628.307585 * t) * t + 0.000349 * cos(4.6261 +
...
double S_aLon_t2(double W) {//已知太阳视黄经反求时间,高速低精度,最大误差不超过600秒
    double t, L, v = 628.3319653318;
    t = (W - 1.75347 - _pi) / v;
    t -= (0.000005297 * t * t + 0.0334166 * cos(4.669257 + 628.307585 * t) +
...
mystl::array2 moonMinR(double t, bool min) {//求月亮近点时间和距离,t为儒略世纪数力学时
...
mystl::array2 moonNode(double t, double asc) {//月亮升交点
...
mystl::array2 earthMinR(double t, bool min) {//地球近远点
...
/*=============一些天文基本问题==============*/
int suoN(double jd) {//返回朔日的编号,jd应在朔日附近，允许误差数天
    return int2((jd + 8) / 29.5306);
}

double sunShengJ(double jd, double L, double fa, int sj) {//太阳升降计算。jd儒略日(须接近L当地平午UT)，L地理经度，fa地理纬度，sj=-1升,sj=1降
    int i;
    ...
    for (i = 0; i < 2; i++) {
        ...
        double J = (48950621.66 + 6283319653.318 * t + 53 * t * t - 994
                    + 334166 * cos(4.669257 + 628.307585 * t) + 3489 * cos(4.6261 + 1256.61517 * t) +
...
double pty_zty(double t) {//时差计算(高精度),t力学时儒略世纪数
    double t2 = t * t, t3 = t2 * t, t4 = t3 * t, t5 = t4 * t;
    double L = (1753470142 + 628331965331.8 * t + 5296.74 * t2 + 0.432 * t3 - 0.1124 * t4 - 0.00009 * t5) / 1000000000 +
               _pi - 20.5 / rad;
    ...
    z[1] = -(2796 * cos(3.1987 + 8433.46616 * t) + 1016 * cos(5.4225 + 550.75532 * t) +
...
double pty_zty2(double t) {//时差计算(低精度),误差约在1秒以内,t力学时儒略世纪数
...
double moonIll(double t) {//月亮被照亮部分的比例
    ...
    a = _pi - D + (-6.289 * sin(m) + 2.100 * sin(M) - 1.274 * sin(D * 2 - m) - 0.658 * sin(D * 2) - 0.214 * sin(m * 2) -
                   0.110 * sin(D)) * dm;
    return (1 + cos(a)) / 2;
}

double moonRad(double r, double h) {//转入地平纬度及地月质心距离,返回站心视半径(角秒)
