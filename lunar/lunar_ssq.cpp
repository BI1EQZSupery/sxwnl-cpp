...
double SSQ::so_low(double W) { //低精度定朔计算,在2000年至600，误差在2小时以内(仍比古代日历精准很多)
...
double SSQ::qi_low(double W) { //最大误差小于30分钟，平均5分
...
double SSQ::qi_high(double W) { //较高精度气
...
double SSQ::so_high(double W) { //较高精度朔
...
int SSQ::calc(double jd, int qs) {// jd应靠近所要取得的气朔日,qs=1定气
    jd += 2451545;
    ...
    if (jd < f1 || jd >= f3) {// 平气朔表中首个之前，使用现代天文算法。1960.1.1以后，使用现代天文算法
        ...
        if (qs == 1)
            ...
            return floor(so_high(floor((jd + pc - 2451551) / 29.5306) * M_PI * 2) +
    ...
    if (jd >= f2 && jd < f3) {// 定气或定朔
        if (qs) {
            ...
            D = floor(so_low(floor((jd + pc - 2451551) / 29.5306) * M_PI * 2) +
                      0.5);    // 2451551是2000.1.7的那个朔日,黄经差为0.定朔计算
...
void SSQ::calcY(double jd) {//农历排月序计算,可定出农历,有效范围：两个冬至之间(冬至一 <= d < 冬至二)
    ...
    if (YY >= -721 && YY <= -104) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            //颁行历年首, 闰月名称, 月建
            if (yy >= -721)
                ns[i] = calc(1457698 - J2000 + int2(0.342 + (yy + 721) * 12.368422) * 29.5306, 0), ns[i + 3] = 0, ns[i +
                                                                                                                     6] = 2;  //春秋历,ly为-722.12.17
            if (yy >= -479)
                ns[i] = calc(1546083 - J2000 + int2(0.500 + (yy + 479) * 12.368422) * 29.5306, 0), ns[i + 3] = 0, ns[i +
                                                                                                                     6] = 2;  //战国历,ly为-480.12.11
            if (yy >= -220)
                ns[i] = calc(1640641 - J2000 + int2(0.866 + (yy + 220) * 12.369000) * 29.5306, 0), ns[i + 3] = 1, ns[i +
                                                                                                                     6] = 11; //秦汉历,ly为-221.10.31
        }
        int nn, f1;
        for (i = 0; i < 14; i++) {
