...
const char *str_jqmc[] = {"冬至", "小寒", "大寒", "立春", "雨水", "惊蛰", "春分", "清明", "谷雨", "立夏", "小满",
...
const char *str_nywx[] = {"海中金", "炉中火", "大林木", "路边土", "剑峰金", "山头火", "涧下水", "城墙土", "白腊金",
                          "杨柳木", "泉中水", "屋上土", "霹雷火", "松柏木", "长流水", "沙中金", "山下火", "平地木",
                          "壁上土", "金泊金", "佛灯火", "天河水", "大泽土", "钗钏金", "桑松木", "大溪水", "沙中土",
                          "天上火", "石榴木", "大海水"};
const char *str_xqmc[] = {"日", "一", "二", "三", "四", "五", "六"};
const char *str_rmc[] = {"初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十", "十一", "十二",
                         "十三", "十四", "十五", "十六", "十七", "十八", "十九", "二十", "廿一", "廿二", "廿三", "廿四",
                         "廿五", "廿六", "廿七", "廿八", "廿九", "三十"};
const char *str_rmc0[] = {"１", "２", "３", "４", "５", "６", "７", "８", "９", "１０", "１１", "１２", "１３", "１４", "１５", "１６", "１７",
                          "１８", "１９", "２０", "２１", "２２", "２３", "２４", "２５", "２６", "２７", "２８", "２９", "３０", "３１"};
const char *str_xz[] = {"摩羯座♑️", "水瓶座♒️", "双鱼座♓️", "白羊座♈️", "金牛座♉️", "双子座♊️", "巨蟹座♋️", "狮子座♌️",
                        "处女座♍️", "天秤座♎️", "天蝎座♏️", "射手座♐️"};
...
void OBA::getDayName(OB_DAY &r) {
    ...
    *****************/
    ...
    //按公历日期查找
    for (i = 0; i < sFtv[r.m - 1].size(); i++) { //公历节日或纪念日,遍历本月节日表
        s = sFtv[r.m - 1][i];
    ...
    //按周查找
    int _w = r.weeki;
    if (r.week >= r.week0) _w += 1;
    int _w2 = _w;
    if (r.weeki == r.weekN - 1) _w2 = 5;
...
void OBA::getHuiLi(double d0, OB_DAY &r) { //回历计算
...
mystl::string OBB::getNH(int y) { //取年号
    int i, j;
    mystl::string c, s = "";
    ...
    for (i = 0; i < ob.size(); i += 7) {
        ...
        s = s + (s.size() ? ";" : "") + "[" + ob[i + 3] + "]" + ob[i + 4] + " " + ob[i + 5] + " " +
            c; //i为年号元年,i+3朝代,i+4朝号,i+5皇帝,i+6年号
    }
    return s;
}

void OBB::getDayName2(OB_DAY &r) { //计算农历节日
    //按农历日期查找重量点节假日
    mystl::string d;
    d = d + r.Lmc + "月" + r.Ldc;
    // std::cout<<d<<std::endl;
...
void OBB::mingLiBaZi(double jd, double J,
                     MLBZ &ob) {                                //命理八字计算。jd为格林尼治UT(J2000起算),J为本地经度,返回在物件ob中
    int i, v;
    double c;
...
double OBB::qi_accurate2(double jd) { //精气
    double d = M_PI / 12;
...
double OBB::so_accurate2(double jd) { //精朔
